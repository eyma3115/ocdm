...
class GaussianDiffusion():
    ...
    def q_sample_conditioning(self, model_kwargs, t, train: bool = True):
        ...
        '''if train:
            if self.conditioning_noise == 'reverse':
                ...
                conditioning_x = model_kwargs["image"] + torch.randn_like(model_kwargs["image"])
        else:'''
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, mean_loss=True):
        ...
        if noise is None:
            noise = torch.randn_like(x_start).to(t.device)
...
class ScheduleSampler(ABC):
    ...
    def sample(self, batch_size, device):
        ...
        #indices = torch.from_numpy(indices_np / len(p)).long().to(device)
